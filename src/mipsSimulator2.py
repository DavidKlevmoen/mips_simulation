...
class MIPSSimulator():
    ...
    def __init__(self, memoryFile):
        ...
        self.elements = [self.constant4, self.adder, self.instructionMemory, self.controls,
                         self.registerFile, self.signExtend, self.shiftLeft16, self.muxReg_ALU, self.aluControl, self.alu,
                         self.dataMemory,
                         self.muxData_Reg, self.muxInstr_Reg, self.registerFile, self.shiftLeft2, self.shiftLeft2Jump,
                         self.addAdder_Shift, self.muxAdder_PC, self.consatenator, self.muxToPc]  # self.muxToPc
    ...
    def _connectCPUElements(self):
        ...
        self.muxAdder_PC.connect([(self.adder, 'sum'), (self.signExtend, 'signExtended')],
                                 ['muxAdder_PCOut'],
                                 [(self.controls, 'branch')],
                                 [])
        ...
        self.controls.connect([(self.instructionMemory, 'instructionOut')],
                              [],
                              [],
                              ['regDst', 'jump', 'branch', 'memRead', 'memToReg', 'aluOp', 'memWrite', 'aluSrc',
                               'regWrite', 'overflow', 'lui'])

        self.registerFile.connect([(self.instructionMemory, 'instructionOut'), (self.muxInstr_Reg, 'muxInstr_RegOut'),
                                   (self.muxData_Reg, 'muxData_RegOut')],
                                  ['readData1', 'readData2'],
        ...
        self.alu.connect([(self.registerFile, 'readData1'), (self.muxReg_ALU, 'muxReg_ALUOut')],
                         ['aluResult'],
                         [(self.aluControl, 'aluControl'), (self.controls, 'branch'), (self.aluControl, 'overflow')],
                         ['aluZero'])
        ...
        self.consatenator.connect(
            [(self.addAdder_Shift, 'addAdder_ShiftOut'), (self.shiftLeft2Jump, 'shiftLeft2JumpOut')],
            ['newPc'],
            [],
            [])
