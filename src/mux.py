...
class MuxInstr_Reg(CPUElement):
    def connect(self, inputSources, outputValueNames, control, outputSignalNames):
        ...
        assert (len(inputSources) == 1), 'Mux should have one input: instruciton'
        assert (len(outputValueNames) == 1), 'Mux has only one output'
        assert (len(control) == 1), 'Mux has one control signal'
        assert (len(outputSignalNames) == 0), 'Mux does not have any control output'
    ...
    def writeOutput(self):
        self.inputZero = self.inputValues[self.input][11:16]
        self.inputOne = self.inputValues[self.input][16:21]
        muxControl = self.controlSignals[self.controlName]
        ...
        if muxControl == 0:
            self.outputValues[self.outputName] = self.inputZero
        else:  # muxControl == 1
            self.outputValues[self.outputName] = self.inputOne
